...
public class HorizontalCalendar: UIView {
    ...
    private lazy var arrow: UIImageView = { [unowned self] in
        ...
        image.image = UIImage(systemName: "icon-arrow-down");
    ...
    private lazy var todayButton: UIButton = { [unowned self] in
        ...
        button.setTitleColor(HorizontalCalendar.todayColor, for: .normal);
...
class DateViewCell: UICollectionViewCell {
    ...
    @objc private func handleTap(_ sender: UITapGestureRecognizer) {
        guard let tappedView = sender.view,
              let index = datesView.subviews.firstIndex(of: tappedView) else {
            return
        };
        select(index);
    }
...
extension UIView {
    func addTapGestureRecognizer(action: @escaping () -> Void) {
        ...
        objc_setAssociatedObject(self, &tapGestureKey, action, .OBJC_ASSOCIATION_RETAIN)
    }
...
// MARK: Useful Date functions and variables
extension Date {
    var day: Int {
